...
impl DerivingGen {
    ...
    fn eq_or_default(&mut self, cmp: Expr<Id<Name>>, def: Expr<Id<Name>>) -> Expr<Id<Name>> {
        let match_id = Id::new(
            ...
            Type::new_op(name("Ordering"), Vec::new()),
            Vec::new(),
        );
        Case(
            box cmp,
            vec![
                Alternative {
                    pattern: Pattern::Constructor(
                        id("EQ", Type::new_op(name("Ordering"), Vec::new())),
                        Vec::new(),
                    ),
                    expression: def,
                },
...
fn compare_tags(lhs: Expr<Id<Name>>, rhs: Expr<Id<Name>>) -> Expr<Id<Name>> {
    ...
    let typ = function_type_(
        var.clone(),
        function_type_(var.clone(), Type::new_op(name("Ordering"), Vec::new())),
    );
