...
use typecheck::{find_specialized_instances, DataTypes, TypeEnvironment, Types};
use types::{extract_applied_type, qualified};
    ...
    fn find_global<'a>(&'a self, name: Name) -> Option<Var<'a>> {
        for class in self.classes.iter() {
            for decl in class.declarations.iter() {
                if decl.name == name {
                    return Some(Var::Class(
                        &decl.typ.value,
                        &*decl.typ.constraints,
                        &class.variable,
                    ));
                }
            }
