...
impl<Id: fmt::Display + AsRef<str>> Type<Id> {
    ...
    pub fn kind(&self) -> &Kind {
        match self {
            ...
            &Type::Application(ref lhs, _) => match lhs.kind() {
...
pub fn try_get_function<'a, Id: AsRef<str>>(
    ...
    match *typ {
        Type::Application(ref xx, ref result) => match **xx {
            Type::Application(ref xx, ref arg) => match **xx {
                Type::Constructor(ref op) if "->" == op.name.as_ref() => Some((&**arg, &**result)),
                _ => None,
            },
            _ => None,
        },
...
impl<I: PartialEq, U: PartialEq> PartialEq for Qualified<Type<I>, U> {
    fn eq(&self, other: &Qualified<Type<I>, U>) -> bool {
        ...
        self.constraints
            ...
            .all(|(l, r)| {
                l.class == r.class && var_eq(&mut mapping, &l.variables[0], &r.variables[0])
            })
