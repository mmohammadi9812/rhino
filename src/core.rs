use interner::*;
use module;
pub use module::LiteralData::{Char, Fractional, Integral, String};
pub use module::{Constructor, DataDefinition, Newtype, TypeDeclaration};
pub use renamer::Name;
use std::fmt;
use typecheck::TcType;
pub use types::Type::{Application, Variable};
pub use types::{Constraint, Qualified, Type, TypeVariable};
...
impl<Ident: Typed<Id = Name>> Typed for Expr<Ident> {
    ...
    fn get_type<'a>(&'a self) -> &'a Type<Name> {
        match self {
            ...
            &Expr::Apply(ref func, _) => match func.get_type() {
...
pub mod translate {
    ...
    impl<'a> Translator<'a> {
                ...
                match expr {
                    module::Expr::Lambda(arg, body) => {
                        ...
                        let l = Lambda(
                            ...
                            box self.translate_expr_rest(*body),
                        );
        ...
        fn translate_expr_rest(&mut self, input_expr: module::TypedExpr<Name>) -> Expr<Id<Name>> {
            ...
            match expr {
                ...
                module::Expr::Apply(func, arg) => Apply(
                    box self.translate_expr(*func),
                    box self.translate_expr(*arg),
                ),
                module::Expr::OpApply(lhs, op, rhs) => {
                    ...
                    Apply(
                        box Apply(box Identifier(Id::new(op, func_type, vec![])), l),
                        r,
                    )
                }
        ...
        fn do_bind_translate(
                ...
                box Case(
                    ...
                    vec![
                        ...
                        Alternative {
                            ...
                            expression: Apply(
                                box fail_ident,
                                box string("Unmatched pattern in let"),
                            ),
                        },
                    ],
                ),
            ...
            Let(
                vec![bind],
                box apply(bind_ident, (vec![expr, Identifier(func_ident)]).into_iter()),
            )
        }
        ...
        fn translate_matching_groups(
            ...
            if bindings.len() == 1 && simple_binding(&bindings[0]) {
                ...
                let expr = {
                    ...
                    let where_bindings_binds = where_bindings.map_or(Vec::new(), |bs| {
            ...
            let equations: Vec<_> = bindings
                ...
                .map(|bind| {
                    ...
                    let where_bindings_binds =
                        where_bindings.map_or(Vec::new(), |bs| self.translate_bindings(bs));
                    (
        ...
        fn next(&mut self) -> Option<&'a Type<Id>> {
            match *self.typ {
                Type::Application(ref lhs, ref rhs) => match **lhs {
                    Type::Application(ref func, _) => match **func {
