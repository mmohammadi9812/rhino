...
impl<Iter: Iterator<Item = char>> Parser<Iter> {
    ...
    pub fn module(&mut self) -> ParseResult<Module> {
        ...
        loop {
            ...
            dbg!("More bindings? {:?}", semicolon.token);
    ...
    fn sub_expression(&mut self) -> ParseResult<Option<TypedExpr>> {
        ...
        dbg!("Begin SubExpr {:?}", self.lexer.current());
    ...
    fn do_binding(&mut self) -> ParseResult<DoBinding> {
        ...
        dbg!("Do binding {:?}", self.lexer.current());
        self.lexer.backtrack();
        let mut lookahead = 0;
        loop {
            lookahead += 1;
            match self.lexer.next().token {
                ...
                _ => {
                    dbg!("Lookahead {:?}", self.lexer.current());
                }
            }
        }
    }
    ...
    fn binary_expression(&mut self, lhs: Option<TypedExpr>) -> ParseResult<Option<TypedExpr>> {
        dbg!("Parse operator expression, {:?}", &self.lexer.current());
...
mod tests {
    ...
    use interner::*;
    use lexer::{Located, Location};
    use parser::*;
    use std::fs::File;
    use std::io::Read;
    use std::path::Path;
    use test::Bencher;
    use typecheck::{apply, case, identifier, if_else, let_, number, op_apply, rational};
